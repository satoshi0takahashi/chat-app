...
RSpec.describe "メッセージ投稿機能", type: :system do
  ...
  context '投稿に失敗したとき' do
    it '送る値が空の為、メッセージの送信に失敗すること' do
      ...
      # 元のページに戻ってくることを確認する
      expect(curernt_path).to eq room_messages_path(@room_user.room)
    end
  end
...
expect{find("input[:name='commit']")}.to change {Message.count}.by[1]
      # 投稿一覧画面に遷移していることを確認する
expext(current_path).to eq room_messages_path(@room_user.room)
...
expect{find('input[:name="commit"]')}.to change{Message.count}.by(1)
      # 送信した値がブラウザに表示されていることを確認する
expext(page).to have_content(message.content)
      # 送信した画像がブラウザに表示されていることを確認する
      expect(page).to have_selector("img")
    end
  end
